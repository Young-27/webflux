...
public class ChainOfThoughServiceImpl implements ChainOfThoughService{
    ...
    public Flux<UserChatResponseDto> getChainOfThoughtResponse(UserChatRequestDto userChatRequestDto) {
        return Flux.create(sink -> {
            ...
            String establishingThoughtChainSystemPropmt = """
            ...
            LlmChatRequestDto llmChatResponseDto = new LlmChatRequestDto(establishingThoughtChainPrompt, establishingThoughtChainSystemPropmt, true, requestModel);
            LlmWebClientService llmWebClientService = llmWebClientServiceMap.get(requestModel.getLlmType());
            Mono<AnswerListResponseDto> cotStepListMono = llmWebClientService.getChatCompletion(llmChatResponseDto)
                    .map(response -> {
...
//            cotStepFlux.map(cotStep -> {
//                String cotStepRequestPrompt = String.format("""
//                        다음은 사용자의 입력입니다: %s
//
//                        사용자의 요구를 다음 단계에 따라 분석해주세요: %s
//                        """, userRequest, cotStep);
//                llmWebClientService.getChatCompletionCatchException();
//            });
        });
    }
}
